...
import { TravelEventType } from "./TravelEvent"; // Import the TravelEventType

interface YearlyCalendarViewProps {
  selectedDate: Date;
  onDateChange: (date: Date) => void;
  onDateSelect: (date: Date) => void;
  events: TravelEventType[]; // Receive the events prop
}
...
const YearlyCalendarView: React.FC<YearlyCalendarViewProps> = ({
  ...
  onDateChange,
  onDateSelect,
  events = [], // Set default empty array
}) => {
  ...
  const renderMonth = (monthName: string, monthIndex: number) => {
    ...
    // Make sure events is defined and is an array before using it
      ...
      <div key={monthName} className="border rounded-lg overflow-hidden bg-white">
        ...
        <div className="grid grid-cols-7 gap-1 p-1">
          {days.map((day, index) => {
            ...
            // Safety check for events before mapping
            const hasUpcomingEvent = safeEvents.some(event =>
            ...
            return (
              <button
                ...
                className={`h-8 flex items-center justify-center rounded-full text-sm transition-colors
                  ...
                  ${hasUpcomingEvent && !isSelected ? 'font-semibold text-blue-600' : ''}
                `}
              >
                {day.getDate()}
              </button>
